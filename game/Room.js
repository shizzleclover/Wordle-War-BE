class Room {
  constructor(id, wordLength, creatorSocketId, creatorUser, options = {}) {
    this.id = id;
  ...
  resetForRematch() {
    ...
    this.startedAt = null;
    this.turnStartedAt = null;
    if (this.blitzTimer) clearTimeout(this.blitzTimer);
    this.blitzTimer = null;
  ...
  usePowerup(socketId, type) {
    ...
    if (type === 'hint') {
      if (player.actionPoints < 3) throw new Error('Not enough AP for Hint');
      player.actionPoints -= 3;
      // Find letter
      const secret = opponent.secretWord;
      const knownCorrect = new Set();
      player.guesses.forEach(g => {
        g.feedback.forEach((f, idx) => {
          if (f === 'correct') knownCorrect.add(idx);
        });
      });
      // Pick random unknown index
      const unknown = [];
      for(let i=0; i<secret.length; i++) {
        if (!knownCorrect.has(i)) unknown.push(i);
      }
      if (unknown.length === 0) return { success: false, message: 'No more hints available!' };
      const rIdx = unknown[Math.floor(Math.random() * unknown.length)];
      return { success: true, type, payload: { index: rIdx, letter: secret[rIdx] } };
    }
    
    if (type === 'scramble') {
      if (player.actionPoints < 4) throw new Error('Not enough AP for Scramble');
      player.actionPoints -= 4;
      return { success: true, type, payload: {} };
    }
    
    if (type === 'blindfold') {
      if (player.actionPoints < 5) throw new Error('Not enough AP for Blindfold');
  ...
  toBlitzTimeoutMatchDocument(winnerSocketId, loserSocketId) {
    const winnerP = this.players.get(winnerSocketId);
    const loserP = this.players.get(loserSocketId);
    ...
    const duration = this.startedAt ? Math.round((Date.now() - this.startedAt.getTime()) / 1000) : 0;
